...
from insights.client import NETWORK
...
class InsightsConnection(object):
    ...
    def _init_session(self):
        ...
        if self.proxy_auth:
            ...
            try:
                ...
                logging.log(NETWORK, "GET %s", self.base_url)
                session.request(
    ...
    def handle_fail_rcs(self, req):
        ...
        if req.status_code >= 400:
            logger.info("Debug Information:\nHTTP Status Code: %s",
                        req.status_code)
            logger.log(NETWORK, "HTTP Status Text: %s", req.reason)
    ...
    def group_systems(self, group_name, systems):
        ...
        logger.log(NETWORK, '/%s/systems' % api_group_id)
